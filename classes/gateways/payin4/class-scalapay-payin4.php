...
class WC_Scalapay_Gateway_Payin4 extends WC_Payment_Gateway {
    ...
    public function __construct() {
        try {
            $this->id = 'wc-scalapay-payin4';
            $this->method_title = __('Scalapay', 'wc-scalapay-gateway');
            $this->has_fields = false;
            $this->supports = array( 'products', 'refunds' );
            $payment_tabs = __('Allow Scalapay Payin4 Payments', 'wc-gateway-scalapay');
            if (isset($_REQUEST['section']) && $_REQUEST['section'] == esc_attr('wc-scalapay-payin4')) {
                ...
                $payment_tabs   .= '<a class="scalapay-widget-link active" href="'.admin_url('admin.php?page=wc-settings&tab=checkout&section=wc-scalapay-payin4').'">SCALAPAY PAY IN 4</a>';
            ...
            $this->init_settings();
            $this->title = __('Scalapay - Pay in 4', 'wc-gateway-scalapay');
    ...
    public function init_form_fields() {
        try {
            $gateway_name   = 'Pay in 4';
            $this->form_fields = include(WC_SCALAPAY_GATEWAY_DIR_PATH.'classes/gateways/payin4/settings-payin4-fields.php');
        } catch (Exception $ex) {
            wc_add_notice($ex->getMessage(),'error');
        }
    }
    ...
    public function init_request_api() {
        try {
            ...
            include_once( WC_SCALAPAY_GATEWAY_DIR_PATH . '/classes/gateways/payin4/class-scalapay-payin4-api-handler.php' );
            $this->api_request_handler = new WC_Scalapay_Gateway_Payin4_API_Handler($this,$api_callback);
        } catch (Exception $ex) {
            wc_add_notice($ex->getMessage(), 'error');
        }
    }
