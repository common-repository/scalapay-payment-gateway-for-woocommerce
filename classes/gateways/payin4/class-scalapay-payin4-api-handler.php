...
class WC_Scalapay_Gateway_Payin4_API_Handler {

    public $Payin4Password;
    public $Payin4Url;
    public $api_callback;
    ...
    public function __construct($gateway,$callback) {
        ...
        if ($this->livemode) {
            $this->Payin4Password = $gateway_settings['production_key']; //$this->gateway->get_option('production_key');
            $this->Payin4Url      = $gateway_settings['production_url']; //$this->gateway->get_option('production_url');
        } else {
            $this->Payin4Password = $gateway_settings['sandbox_key'];
            $this->Payin4Url      = $gateway_settings['sandbox_url'];
        }
    }

    public function request_return_handler($order_id) {
        try {
            $ScalapayUrl = esc_url($this->Payin4Url.'/v2/orders');
            $ScalapayPassword = esc_attr($this->Payin4Password);
            ...
            if ($this->log_enabled == esc_attr('yes')) {
                wc_scalapay_log('wc-scalapay-payin4', 'request_return_handler -> get_order_details', $order_data);
            }
             ...
             */
            if ($this->log_enabled == esc_attr('yes')) {
                wc_scalapay_log('wc-scalapay-payin4', 'response_request_return_handler', json_encode($response));
            }
    ...
    public function get_order_details($order_id) {
        ...
        $confirm_url = $this->api_callback . '/?scalapay-payin4=true&order_id=' . $cart_session_id . '&total=' . $grandTotal;
        $cancel_url = $this->api_callback . '/?scalapay-payin4=cancel';
    ...
    public function request_check_response() {
        try {
            if (isset($_REQUEST['scalapay-payin4'])) {
                if ($_REQUEST['scalapay-payin4'] == esc_attr('cancel') && $_REQUEST['status'] == esc_attr('FAILED'))
                {
                ...
                if (!empty($_REQUEST['orderToken']) && $_REQUEST['status'] == esc_attr('SUCCESS'))
                {
                    ...
                    $response_status = $_REQUEST['status'];
                     ...
                     */
                    if ($this->log_enabled == esc_attr('yes')) {
                        wc_scalapay_log('wc-scalapay-payin4', 'request_check_response -> capture_scalapay_order', json_encode($captureResponse));
                    }
                    ...
                    if (isset($captureResponse["status"]) && $captureResponse["status"] == "APPROVED") {
                         ...
                         */
                        if ($this->log_enabled == esc_attr('yes')) {
                            wc_scalapay_log('wc-scalapay-payin4', 'request_check_response -> scalapay_send_orderID', $response);
                        }
    ...
    public function scalapay_send_orderID($order_id, $orderToken) {
        $scalapay_gateway_url = esc_url($this->Payin4Url.'/v2/orders/'.$orderToken);
        $scalapay_password    = esc_attr($this->Payin4Password);
    ...
    public function capture_scalapay_order($order_id, $orderToken) {
        $scalapay_gateway_url = esc_url($this->Payin4Url.'/v2/payments/capture');
        $scalapay_password    = esc_attr($this->Payin4Password);
    ...
    public function create_refund($order, $amount, $order_id) {
        ...
        $ScalapayUrl       = esc_url($this->Payin4Url.'/v2/payments/'.$order_token.'/refund');
        $ScalapayPassword  = esc_attr($this->Payin4Password);
         ...
         */
        if ($this->log_enabled == esc_attr('yes')) {
            wc_scalapay_log('wc-scalapay-payin4', 'request_process_refund -> create_refund',json_encode($res));
        }
