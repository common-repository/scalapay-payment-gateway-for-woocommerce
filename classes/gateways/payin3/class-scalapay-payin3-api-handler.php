...
class WC_Scalapay_Gateway_Payin3_API_Handler {
    public $Payin3Password;
    public $Payin3Url;
    public $api_callback;
    ...
    public function __construct($gateway,$callback) {
        ...
        if ($this->livemode) {
            $this->Payin3Password = $this->gateway->get_option('production_key');
            $this->Payin3Url      = $this->gateway->get_option('production_url');
        } else {
            $this->Payin3Url      = $this->gateway->get_option('sandbox_url');
            $this->Payin3Password = $this->gateway->get_option('sandbox_key');
        }
    }

    public function request_return_handler($order_id) {
        try {
            $ScalapayUrl = esc_url($this->Payin3Url.'/v2/orders');
            $ScalapayPassword = esc_attr($this->Payin3Password);
            ...
            if ($this->log_enabled == esc_attr('yes')) {
                wc_scalapay_log('wc-scalapay-payin3', 'request_return_handler -> get_order_details', $order_data);
            }
             ...
             */
            if ($this->log_enabled == esc_attr('yes')) {
                wc_scalapay_log('wc-scalapay-payin3', 'response_request_return_handler', json_encode($response));
            }
            return $response;
    ...
    public function get_order_details($order_id) {
        ...
        $confirm_url = $this->api_callback . '/?wc-scalapay-payin3=true&order_id=' . $cart_session_id . '&total=' . $grandTotal;
        $cancel_url = $this->api_callback . '/?wc-scalapay-payin3=cancel';
    ...
    public function request_check_response() {
        try {
            if (isset($_REQUEST['wc-scalapay-payin3']) && !empty($_REQUEST['orderToken'])) {
                 ...
                 */
                if ($this->log_enabled == esc_attr('yes')) {
                    wc_scalapay_log('wc-scalapay-payin3', 'request_check_response -> capture_scalapay_order', json_encode($captureResponse));
                }

                if (isset($captureResponse["errorCode"]) && $captureResponse["errorCode"] != 1) {
                    if (isset($captureResponse["message"])) {
                        $error_message = $captureResponse["message"];
                    }
                    wc_add_notice(__($error_message, 'gateway'), 'error');
                    ...
                    if (isset($captureResponse["status"]) && $captureResponse["status"] == "APPROVED") {
                         ...
                         */
                        if ($this->log_enabled == esc_attr('yes')) {
                            wc_scalapay_log('wc-scalapay-payin3', 'request_check_response -> scalapay_send_orderID', $response);
                        }
                ...
                wc_add_notice(__($this->scalapay_failed_message, 'gateway'), 'error');
                wp_safe_redirect(wc_get_page_permalink('cart'));
            }
        } catch (Exception $ex) {
    ...
    public function scalapay_send_orderID($order_id, $orderToken) {
        $scalapay_gateway_url = esc_url($this->Payin3Url.'/v2/orders/'.$orderToken);
        $scalapay_password    = esc_attr($this->Payin3Password);
    ...
    public function capture_scalapay_order($order_id, $orderToken) {
        $scalapay_gateway_url = esc_url($this->Payin3Url.'/v2/payments/capture');
        $scalapay_password    = esc_attr($this->Payin3Password);
    ...
    public function create_refund($order, $amount, $order_id) {
        ...
        $ScalapayUrl       = esc_url($this->Payin3Url.'/v2/payments/'.$order_token.'/refund');
        $ScalapayPassword  = esc_attr($this->Payin3Password);
         ...
         */
        if ($this->log_enabled == esc_attr('yes')) {
            wc_scalapay_log('wc-scalapay-payin3', 'request_process_refund -> create_refund',json_encode($res));
        }
